...
class ViViT_SLR(nn.Module):
    def __init__(self,
        ...
        self.adapter_normal_to_kp = nn.ModuleList([Adapter() for _ in range(12)])
        self.adapter_kp_to_normal = nn.ModuleList([Adapter() for _ in range(12)])

        self.adapter_norm_normal = nn.ModuleList([nn.LayerNorm(768, eps=1e-6) for _ in range(12)])
        self.adapter_norm_kp = nn.ModuleList([nn.LayerNorm(768, eps=1e-6) for _ in range(12)])
    ...
    def forward(self, normal_vid, keypoint_vid, y_tokens):
        ...
        for i in range(12):
            ...
            normal_encoded_output = self.adapter_norm_normal[i](
                normal_encoded_output + self.adapter_kp_to_normal[i](keypoint_encoded_output)
            )

            keypoint_encoded_output = self.adapter_norm_kp[i](
                keypoint_encoded_output + self.adapter_normal_to_kp[i](normal_encoded_output)
            )
