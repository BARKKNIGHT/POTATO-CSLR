class Adapter(nn.Module):
    def __init__(self, dim=768, bottleneck=64):
        ...
        self.act  = nn.ReLU()
        self.up   = nn.Linear(bottleneck, dim)  # Expand back

    def forward(self, x_self, x_other):
        return x_self + self.up(self.act(self.down(x_other)))  # Residual connection
...
class ViViT_SLR(nn.Module):
    def __init__(self,
        ...
        normal_to_kp_fusion_blocks_list = []
        kp_to_normal_fusion_blocks_list = []

        for n in range(12):
            if n == self.early_attention or n == self.mid_attention:
                normal_to_kp_fusion_blocks_list.append(nn.MultiheadAttention(embed_dim=768, num_heads=num_heads, dropout=dropout, batch_first=batch_first))
                kp_to_normal_fusion_blocks_list.append(nn.MultiheadAttention(embed_dim=768, num_heads=num_heads, dropout=dropout, batch_first=batch_first))

            else:
                normal_to_kp_fusion_blocks_list.append(Adapter())
                kp_to_normal_fusion_blocks_list.append(Adapter())


        self.normal_to_kp_fusion_blocks = nn.ModuleList(normal_to_kp_fusion_blocks_list)
        self.kp_to_normal_fusion_blocks = nn.ModuleList(kp_to_normal_fusion_blocks_list)

        self.norm_normal_flow = nn.ModuleList([nn.LayerNorm(768, eps=1e-6) for _ in range(12)])
        self.norm_kp_flow = nn.ModuleList([nn.LayerNorm(768, eps=1e-6) for _ in range(12)])
    ...
    def forward(self, normal_vid, keypoint_vid, y_tokens):
        ...
        for i in range(12):
            ...
            if i == self.early_attention or i == self.mid_attention:

                if i == self.early_attention:
                    normal_encoded_output = self.early_norm_normal_to_kp(normal_encoded_output)
                    keypoint_encoded_output = self.early_norm_kp_to_normal(keypoint_encoded_output)

                else:
                    normal_encoded_output = self.mid_norm_normal_to_kp(normal_encoded_output)
                    keypoint_encoded_output = self.mid_norm_kp_to_normal(keypoint_encoded_output)


                normal_encoded_output = self.norm_normal_flow[i](
                    normal_encoded_output + self.normal_to_kp_fusion_blocks[i](query=normal_encoded_output, key=keypoint_encoded_output, value=keypoint_encoded_output)[0]
                )

                keypoint_encoded_output = self.norm_kp_flow[i](
                    keypoint_encoded_output + self.kp_to_normal_fusion_blocks[i](query=keypoint_encoded_output, key=normal_encoded_output, value=normal_encoded_output)[0]
                )

            else:

                normal_encoded_output = self.norm_normal_flow[i](
                    self.normal_to_kp_fusion_blocks[i](normal_encoded_output, keypoint_encoded_output)
                )

                keypoint_encoded_output = self.norm_kp_flow[i](
                    self.kp_to_normal_fusion_blocks[i](keypoint_encoded_output, normal_encoded_output)
                )
